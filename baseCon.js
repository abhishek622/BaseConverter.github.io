function doBaseCon() {
  ...
  if (check(to, 0) == 1 || check(from, 0) == 1) {
    form.res.value = 'Bases must be integer (excluding -1, 0, +1).';
    return;
  }
  if (to == 0 || from == 0) {
    form.res.value = 'That base is a mathematical impossibility.';
    return;
  }
  if (to == -1 || to == 1 || from == -1 || from == 1) {
    form.res.value = "I don't think that base will work so well.";
    return;
  }
